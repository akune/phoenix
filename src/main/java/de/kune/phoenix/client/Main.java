	...
	public void onModuleLoad() {
		...
		generator.init(new Callback<Void, Exception>() {
			...
			public void onSuccess(Void result) {
				generator.createAsync("cbVz3B4xbxx2E3l4l0LeCnjD7GkFXyshtxxhtnN5hF8sUVjmdD0zYAQgO7ca2pN4pazU5R7xx1EGr80jZBW5WZH0yA", "4wxbdmWP5JYXXdxxfq3fqHa8BunOHCGib0m5dwRkar1J8xajgNeVTVUnZdPzpuAX6xxPUVlbcXLRBuhpHWxaE2bPIs5ZI0tQxxsaRIgukrXEpmw4gPCVmzNSkwfopBm8xx2gCZ3pRDnMEvfXhxbJTQ5xxgxxyfkQ", new Callback<RsaKeyPair, Exception>() {
					...
					public void onSuccess(RsaKeyPair result) {
						try {
							...
							byte[] encrypted = result.encrypt(RsaKeyPair.KeyType.PUBLIC, "0123456789abcde".getBytes("UTF-8"));
				...
				generator.generateKeyPairAsync(KeyStrength.WEAKEST, PublicExponent.SMALLEST, new Callback<RsaKeyPair, Exception>() {
					@Override
					public void onSuccess(RsaKeyPair result) {
						welcomeLabel.setText(welcomeLabel.getText() + "done");
						panel.add(new Anchor("Use generated key pair for encryption / decryption",
								"titaniumcore/encryption.html?pu=" + result.getEncodedPublicKey() + "&pr=" + result.getEncodedPrivateKey()
										+ "",
								"_blank"));
						GWT.log("done, state=" + generator.getState());
						...
						try {
							byte[] encrypted = result.encrypt(RsaKeyPair.KeyType.PUBLIC, "Plain Text".getBytes("UTF-8"));
							GWT.log("Encrypted: " + Arrays.toString(encrypted));
							GWT.log(new String(result.decrypt(RsaKeyPair.KeyType.PRIVATE, encrypted), "UTF-8"));
						} catch (UnsupportedEncodingException e) {
							e.printStackTrace();
					...
					public void onSuccess(Integer progress) {
						// GWT.log("progressing, state=" + generator.getState());
						if (progress % 500 == 0) {
