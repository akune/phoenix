...
public class Sha256 implements Digest {
	...
	private int[] state = { 0x6A09E667, 0xBB67AE85, 0x3C6EF372, 0xA54FF53A, 0x510E527F, 0x9B05688C, 0x1F83D9AB,
			0x5BE0CD19 };
	...
	private void processBlock(byte[] m, int pos) {
		...
		for (int j = 0; j < BLOCK_SIZE; j++, pos += 4) {
			if (j < 16) {
				w[j] = ((m[pos + 0] & 0xFF) << 24) | ((m[pos + 1] & 0xFF) << 16) | ((m[pos + 2] & 0xFF) << 8)
						| ((m[pos + 3] & 0xFF) << 0);
			} else {
				w[j] = safeAdd(safeAdd(safeAdd(gamma1256(w[j - 2]), w[j - 7]), gamma0256(w[j - 15])), w[j - 16]);
			}

			int T1 = safeAdd(safeAdd(safeAdd(safeAdd(h, sigma1256(e)), ch(e, f, g)), K[j]), w[j]);
	...
	@Override
	public Digest feed(String input, String charset) throws UnsupportedEncodingException {
		return feed(input.getBytes(charset));
	}

	@Override
	public Digest feed(String input) {
		try {
			return feed(input, "UTF-8");
	...
	public String toBase64() {
		return Base64Utils.encode(toByteArray());
	}
	...
	@Override
	public ByteArraySource iterate(int iterations) {
		byte[] result = toByteArray();
		for (int i = 0; i < iterations; i++) {
			result = new Sha256().feed(result).toByteArray();
		}
		...
		return new ByteArraySource() {
			...
			@Override
			public String toBase64() {
				return Base64Utils.encode(finalResult);
			}

			@Override
			public byte[] toByteArray(byte[] salt) {
				return concat(salt, finalResult);
			}

			@Override
			public String toBase64(byte[] salt) {
				return Base64Utils.encode(toByteArray(salt));
			}

		};
	}

	private static byte[] concat(byte[] salt, final byte[] finalResult) {
		byte[] result = new byte[salt.length + finalResult.length];
		for (int i = 0; i < salt.length; i++) {
			result[i] = salt[i];
		}
		for (int i = 0; i < finalResult.length; i++) {
			result[i + salt.length] = finalResult[i];
		}
		return result;
	}
	...
	@Override
	public byte[] toByteArray(byte[] salt) {
		return concat(salt, toByteArray());
	}
